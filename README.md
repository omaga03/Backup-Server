...
*   **Smart Sync**:
    *   **Incremental Sync**: ดึงเฉพาะไฟล์ที่มีการเปลี่ยนแปลง เพื่อความรวดเร็ว
    *   **Manual Full Wipe**: สามารถเลือกโหมดลบข้อมูลเก่าทั้งหมดแล้วดึงใหม่ได้ (กด 1 ตอนเริ่มโปรแกรม)
*   **Backup Retention Policy (ระบบหมุนเวียนไฟล์)**:
    *   **Normal Day**: ระบบจะเก็บไฟล์สำรองข้อมูลไว้เพียง 1 ชุดต่อวัน (หมุนเวียนทับไฟล์เดิมของวันนั้นๆ)
    *   **Retention Day (วันเก็บสะสม)**: ค่าเริ่มต้นคือ **วันศุกร์ (Fri)** ซึ่งระบบจะ **ไม่ลบ** ไฟล์ของวันนี้ทิ้ง ทำให้มี Weekly Archive สะสม (ตั้งค่าได้ที่ `retention_day`)
